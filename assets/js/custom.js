...
function _asyncToGenerator(fn) { return function () { var self = this, args = arguments; return new Promise(function (resolve, reject) { var gen = fn.apply(self, args); function _next(value) { asyncGeneratorStep(gen, resolve, reject, _next, _throw, "next", value); } function _throw(err) { asyncGeneratorStep(gen, resolve, reject, _next, _throw, "throw", err); } _next(undefined); }); }; }
jQuery(function ($) {
  var categoryInput = ".post_categories";
  var postsWrapper = $(".posts_grid_inner");
  var loadMore = $(".load-more-filter");
  var childCategoriesWrapper = $('.content_filter .posts_filter_bottom');
  ...
  var syncParamsWithUrl = function syncParamsWithUrl() {
    ...
    if (url.searchParams.get('categoriesChildren') !== null) {
      var selectedCategoriesChildrenUrl = url.searchParams.get('categoriesChildren').split('-');
      if (selectedCategoriesChildrenUrl) {
        selectedCategoriesChildrenUrl.forEach(function (child) {
          selectedCategoriesChildren.push({
            parentID: $("#".concat(child)).data('parent'),
            ID: child
          });
        });
      }
  ...
  var visualPanel = function visualPanel(selectedCategories, selectedCategoriesChildren) {
    ...
    if (selectedCategories.length !== 0) {
      selectedCategories.map(function (catID) {
        var visualElem = $('<span> <span class="filter_panel_e_delete">&#10005;</span></span>');
  ...
  var filterAjaxRequest = /*#__PURE__*/function () {
    ...
    return function filterAjaxRequest() {
      return _ref.apply(this, arguments);
    };
  }();
  var urlApi = function urlApi(params) {
    var childParams = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
    ...
    if (childParams.length !== 0) {
      var childParamsIDs = childParams.map(function (child) {
        return child.ID;
      });
  ...
  var filter = function filter(e) {
    ...
    if (current.data('child') == true) {
      if (checked === true) {
        ...
        selectedCategoriesChildren = selectedCategoriesChildren.filter(function (child) {
      ...
      if (checked === true) {
        ...
        var indexToDelete = selectedCategories.indexOf(categoryID);
        if (indexToDelete !== -1) {
